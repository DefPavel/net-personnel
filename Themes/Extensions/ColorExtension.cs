...
public static class ColorExtension
{
    ...
    {
        if (Application.Current.TryFindResource(AccentBrush) is SolidColorBrush accentColor)
        {
            return;
        }
        ...
        ResourceDictionary? applicationResources = application.Resources;
    ...
    {
        Color c = Color.FromArgb(180, color.R, color.G, color.B);
        return c.MakeOpaque();
    }

    private static Color GetMouseOverBackgroundColor(this Color color)
    {
        Color c = Color.FromArgb(30, color.R, color.G, color.B);
        return c.MakeOpaque();
    }

    private static Color GetMouseOverBorderColor(this Color color)
    {
        Color c = Color.FromArgb(70, color.R, color.G, color.B);
        return c.MakeOpaque();
    }
    ...
    {
        Color c = Color.FromArgb(110, color.R, color.G, color.B);
        return c.MakeOpaque();
    }
    ...
    {
        Color c = Color.FromArgb(200, color.R, color.G, color.B);
        return MixColors(c, Colors.Black);
    }
    ...
    {
        int r = (color1.R * color1.A / 255) + (color2.R * color2.A * (255 - color1.A) / (255 * 255));
        int g = (color1.G * color1.A / 255) + (color2.G * color2.A * (255 - color1.A) / (255 * 255));
        int b = (color1.B * color1.A / 255) + (color2.B * color2.A * (255 - color1.A) / (255 * 255));
